...
export const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ 
  ...
  useEffect(() => {
    const unsubscribe = store.subscribe(() => {
      setProjects(store.getUserProjects(currentUser));
  ...
  // Pending payments from sent quotes
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 p-8 sm:p-10 text-white shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-300 max-w-xl leading-relaxed">
            Manage your digital projects, review interactive milestones, accept official quotations, and communicate with the NSK engineering team.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-card flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-[11px] text-amber-600 font-semibold">Under Scope Review</div>
          </div>
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-card flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-[11px] text-slate-500 font-semibold">Awaiting Your Approval</div>
          </div>
      ...
      {/* Projects & Requests Table / Card List */}
      <div className="rounded-3xl bg-white border border-slate-200/90 shadow-card p-6 sm:p-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900">Your Project Requests & Orders</h2>
            <p className="text-xs text-slate-500 mt-0.5">Click on any project to view milestones, real-time chat, and files.</p>
          </div>
        ...
        {filteredProjects.length === 0 ? (
          <div className="py-16 text-center space-y-4">
            ...
            <h3 className="text-base font-bold text-slate-900">No project requests found</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              You haven’t submitted any project requests yet. Click below to get a custom quotation and start your project.
            </p>
            <button
              onClick={onRequestNew}
              className="px-5 py-2.5 rounded-xl bg-blue-600 text-white font-bold text-xs shadow-sm hover:bg-blue-700 transition-all"
            >
